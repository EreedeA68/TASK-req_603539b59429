...
cleanup() {
    ...
    $DC -f "$COMPOSE_FILE" down -v 2>/dev/null || true
}
trap cleanup EXIT

# Step 1: Build test Docker image
echo ""
echo "[1/7] Building test Docker image..."
$DC -f "$COMPOSE_FILE" build --no-cache

# Step 2: Start test environment
echo ""
echo "[2/7] Starting test environment (app + db)..."
$DC -f "$COMPOSE_FILE" up -d

# Step 3: Wait for app health check
echo ""
echo "[3/7] Waiting for app to be healthy (max ${MAX_WAIT}s)..."
elapsed=0
until curl -sf "$HEALTH_URL" > /dev/null 2>&1; do
    if [ "$elapsed" -ge "$MAX_WAIT" ]; then
        echo "ERROR: App did not become healthy within ${MAX_WAIT}s"
        $DC -f "$COMPOSE_FILE" logs app-test
        exit 1
    fi
